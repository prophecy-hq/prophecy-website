...
export default function Art() {
	...
	useEffect(() => {
		...
		const fragmentShader = `uniform vec3 vertColor; 
    uniform float alpha;
    ...
    void main() {
		...
		Line.prototype = {
			...
			drawLine: function () {
				...
				targetPoints.push(this.x1);
				targetPoints.push(this.y1);
				targetPoints.push(Math.random());
				targetPoints.push(this.x2);
				targetPoints.push(this.y2);
				targetPoints.push(Math.random());
				...
				points.push(cx + Math.cos(ang + Math.PI / 2) * des);
				points.push(cy + Math.sin(ang + Math.PI / 2) * des);
				points.push(Math.random());
				targetPoints.push(cx + Math.cos(ang + Math.PI / 2) * des);
				targetPoints.push(cy + Math.sin(ang + Math.PI / 2) * des);
				targetPoints.push(Math.random());
		...
		function mainAnimation(i) {
			...
			if (currentDistance / targetDistance < minLengthProgress) {
				directionIsReverse[i] = false;
			}
			if (currentDistance / targetDistance > maxLengthProgress) {
				directionIsReverse[i] = true;
			}
		...
		function opacityAnimation(i) {
			...
			if (currentAlpha < minAlpha) {
				alphaIsIncreasing[i] = true;
			}
			if (currentAlpha > maxAlpha) {
				alphaIsIncreasing[i] = false;
			}

			if (alphaIsIncreasing[i]) {
				lines.material.uniforms.alpha.value +=
