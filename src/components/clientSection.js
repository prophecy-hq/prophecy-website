...
export default function ClientSection() {
	...
	useEffect(() => {
		...
		var arr = heading.querySelectorAll(
			'#clientSectionHeading .word > .char, .whitespace',
		);
		...
		gsap
			...
			.to(
				clientsArr,
				{
					duration: timelineSettings.clientsDuration,
					ease: 'Power3.easeOut',
					opacity: 1,
		...
		<Flex ref={ref} sx={{ mt: '20vh', color: 'lightRed' }}>
			<Flex sx={{ alignItems: 'top', variant: 'layouts.responsiveFlex' }}>
				...
				>
					<h1
						sx={{ variant: 'text.h1' }}
						id="clientSectionHeading"
						data-splitting=""
					>
						We bring in experience from startups and established companies.
					</h1>
					<div
						id="clientSectionSubheading"
						sx={{ color: 'white', variant: 'text.bodyLarge', mt: 32 }}
					>
						Working in healthcare, finance, education and enterprise
						applications.
					</div>
				</Box>
